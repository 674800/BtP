...
public class IndexTitleScrollView extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < chars.length; i++) {
			...
			canvas.drawRect(0, (float) i * iHeight, getRight(), iHeight
					* (float) (i + 1), paint);
			...
			if (isCicle) {
				if (i == mIndex - 2) {
					/*canvas.drawText(chars[i] + "", getLeft() + 10 + 2 * iHeight
							* (float) 0.5, (float) i * iHeight + 15, paint);*/
					canvas.drawText(chars[i] + "", getLeft() + 20, (float) i * iHeight + 15,
							paint);
				} else if (i == mIndex - 1) {
					canvas.drawText(chars[i] + "", getLeft() + 30, (float) i * iHeight + 15,
							paint);
				} else if (i == mIndex) {
					canvas.drawText(chars[i] + "",
							getLeft() + 40, (float) i * iHeight
									+ 15, paint);
				} else if (i == mIndex + 1) {
					canvas.drawText(chars[i] + "", getLeft() + 30, (float) i * iHeight + 15,
							paint);
					...
					canvas.drawText(chars[i] + "", getLeft() + 10, (float) i
							* iHeight + 15, paint);
				}
			} else {
				canvas.drawText(chars[i] + "", getLeft() + 10, (float) i
